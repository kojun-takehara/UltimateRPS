...
namespace RuntimeGizmos
{
	...
	public class TransformGizmo : MonoBehaviour
	{
		...
		{
			// Force transformType to Rotate only
		...
		IEnumerator TransformSelected(TransformType transType)
		{
			...
			{
				// Calculate the current mouse position on the plane
				...
				if(previousMousePosition != Vector3.zero && mousePosition != Vector3.zero)
				{
					...
					// Apply the rotation to the selected objects
					for(int i = 0; i < targetRootsOrdered.Count; i++)
					{
					...
					// Accumulate the rotation amount
			...
			// Store the new transformation state for undo/redo
			...
			// Update the pivot point and axis information to reflect the rotated state
			SetPivotPoint();
			SetAxisInfo();
		}
		...
		void GetTarget()
		{
			if (nearAxis == Axis.None && Input.GetMouseButtonDown(0)) // Left-click for selection
			{
				RaycastHit hitInfo;

				if (Physics.Raycast(myCamera.ScreenPointToRay(Input.mousePosition), out hitInfo, Mathf.Infinity, selectionMask))
				{
					Transform newTarget = hitInfo.transform;

					if (mainTargetRoot != newTarget)
					{
						ClearTargets();  // Clear current selection
						ClearAndAddTarget(newTarget);  // Add the new selection
					}
				}
				else
				{
					// Clear the selection if no object was clicked
					ClearTargets();
				}
			}
			else if (Input.GetMouseButtonDown(1)) // Right-click for reset rotation
			{
				RaycastHit hitInfo;
				...
				if (Physics.Raycast(myCamera.ScreenPointToRay(Input.mousePosition), out hitInfo, Mathf.Infinity, selectionMask))
				{
					Transform clickedTarget = hitInfo.transform;

					// Reset rotation of the clicked object if it is currently selected
					if (mainTargetRoot == clickedTarget)
					{
						ResetRotation();
					}
				}
		...
		public void ClearTargets(bool addCommand = true)
		{
			...
			ClearAllHighlightedRenderers();
			targetRoots.Clear();
			targetRootsOrdered.Clear();  // This will effectively make mainTargetRoot return null
			children.Clear();

			SetPivotPoint();  // Optionally reset the pivot
		}
		...
		void SetNearAxis()
		{
			...
			float handleMinSelectedDistanceCheck = (this.minSelectedDistanceCheck + handleWidth) * distanceMultiplier * 0.2f;
		...
		void ResetRotation()
		{
			if (mainTargetRoot != null)
			{
				mainTargetRoot.localEulerAngles = Vector3.zero; // Reset rotation on all axes to (0, 0, 0)
				totalRotationAmount = Quaternion.identity; // Reset cumulative rotation for the gizmo
				SetAxisInfo(); // Update the gizmo's axis to match the reset rotation
			}
		}
		...
		void HandleNearest(TransformType type, float xClosestDistance, float yClosestDistance, float zClosestDistance, float allClosestDistance, float minSelectedDistanceCheck)
		{
			...
			else if(type == TransformType.Rotate && mainTargetRoot != null)
			{
				...
				if((pivotPoint - mousePlaneHit).sqrMagnitude <= (GetHandleLength(TransformType.Rotate) * 0.2f).Squared()) SetTranslatingAxis(type, Axis.Any);
			}
		}
		...
		void SetAxisInfo()
		{
			if (mainTargetRoot != null)
			{
				...
				// Apply rotation only during transformation or when in Global space
		...
		void SetCircles(AxisInfo axisInfo, AxisVectors axisVectors)
		{
			...
			{
				float circleLength = GetHandleLength(TransformType.Rotate) * 0.2f;
